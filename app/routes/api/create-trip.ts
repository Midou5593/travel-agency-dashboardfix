...
export const action = async ({ request }:ActionFunctionArgs) => {
   ...
   // use the data to generate the trip using GoogleGenerative AI
   // return the trip as a json object
   ...
   try {
    ...
    const testResult = await genAI
         .getGenerativeModel({model: "gemini-2.0-flash"})
         .generateContent([prompt]);

     const trip = parseMarkdownToJson(testResult.response.text());
